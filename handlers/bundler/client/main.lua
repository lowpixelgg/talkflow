...
function initializeVoipPlayer ()
  VOIP = voip:create(VoiceSettings); 
  ...
  -- Radio channels
	...
	-- doPulse ts3 player plugin_data
...
function addPlayerToRadio(channel, isRadio) 
	network:emit("pw_voice:addPlayerToRadio", true, false, channel, localPlayer, isRadio);
end


function removePlayerFromRadio(channel) 
	network:emit("pw_voice:addPlayerToRadio", true, false, channel, VOIP.serverId)
end